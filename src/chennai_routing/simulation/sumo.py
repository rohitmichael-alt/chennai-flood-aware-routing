...
def netconvert_binary() -> str:
    """Resolve netconvert, including an explicit portable-binary override."""

    explicit = os.environ.get("SUMO_NETCONVERT", "").strip()
    binary = explicit if explicit and Path(explicit).is_file() else shutil.which("netconvert")
    if binary is None:
        raise RuntimeError(
            "netconvert is required for Stage 5 SUMO import; set "
            "SUMO_NETCONVERT when using a portable SUMO archive."
        )
    return binary
